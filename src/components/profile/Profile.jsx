...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`https://taskmanagerapprock2.onrender.com/api/auth/profile`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
  ...
  const updateProfile = async () => {
    try {
      await axios.put(`https://taskmanagerapprock2.onrender.com/api/auth/profile/`, editedUser, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
